...
<div id="content_container">
  ...
  <div id="content">
	...
	You may also want to configure other parameters default settings. LSC uses external aligners to find short reads to long reads mappings. Latest LSC version supports <a href="http://bio-bwa.sourceforge.net/">BWA</a>, 
	...
	The aligners and their setting options could be modified through run.cfg file. The path to selected aligner should be set in your PATH env variable.</li>
    
    <li>Execute "<a href="LSC_manual.asp#runLSC">/home/user/LSC_path/runLSC.py run.cfg</a>" while in your working directory. or Execute "<a href="LSC_manual.asp#runLSC">runLSC.py run.cfg</a>, if all LSC executable files are in the default bin </li>
    
    <li>After <a href="LSC_manual.asp#time">a certain time</a> execution will conclude. You can find results in the "output" directory. </li>
    ...
    Its options are described in <a href="LSC_cfg.asp">run.cfg</a> file. You just need to run "runLSC.py" with a configuration file "run.cfg": </p>
    <blockquote>
	...
	<ul><li>mode 0 (default): end-to-end LSC run.</li>
	    <li>mode 1: generates compressed SR to LR alignment file (SR.fa.cps.nav) which is used in LSC correctiopn step.</li>
		<li>mode 2: runs LSC correction step. Note:  make sure SR.fa.cps.nav file is already generated in temp folder.
		            At this stage, LSC also uses some other intermediate files in temp folder which are generated after mode 0/1 run </li>
	</ul>
	</br>
	Other LSC options:
