...
<div id="content_container">
  ...
  <div id="content">
    ...
    <tr>
    <td>
    <a href="files/IDP_0.1.6.tar.gz"  onClick="ga('send', 'event', 'file download', 'click', 'IDP download release');"><img src="images/download.png"><b>IDP 0.1.6</b></a>
    </td>
     <td>
    Please see the <a href="IDP_download_016.asp#notes">release notes</a> for 
    more information
    </td>
    </tr>
    ...
    <div id="notes">
    <h3><img src="images/dna.png">  <b>IDP 0.1.6</b> - Release Notes</h3>
    </div>
   <p>
   IDP integrates short reads (e.g. Illumina data) and long reads (e.g. PacBio data) to identify gene isoforms (transcripts) from transcriptome (see Figure above).<br>
    ...
    <h3>Changes in version 0.1.6<br></h3>
<ul>
  <li> The IDP software is now licensed under Apache 2.0 (a very open license) </li>
  <li> BLAT and seqmap aligners are no longer bundled.  Paths to aligner executables must be specificed in the config file if they are not installed under their default names.</li>
  <li> GMAP can now be used rather than BLAT.  This is  by setting 'aligner_choice' to either 'gmap' or 'blat' in the config file. GMAP also requires the folder holding the index be specified in the config file.</li>
  <li> An option to use MLE rather than Maximum a posteriori probability (MAP) is available by setting 'estimator_choice' to 'MAP' or 'MLE' in the config file.  MAP is used by default, but in data sets with few long reads where few isoforms are detected, MLE should be used.</li>
  <li> A bug was fixed where in the previous version, where IDP should have generated the file when 'detected_exp_len' whas left blank but did not.</li>
</ul>
